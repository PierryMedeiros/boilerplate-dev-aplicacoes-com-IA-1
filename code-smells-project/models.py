...
# SMELL: "Models" que são apenas funções soltas, sem classes ou encapsulamento
# SMELL: Mistura de lógica de negócio com acesso a dados
# SMELL: SQL concatenado com strings (SQL Injection)
# SMELL: Nenhuma validação nos models
...
def get_todos_produtos():
    ...
    result = []
    for row in rows:
        # SMELL: Conversão manual repetitiva ao invés de usar um serializer
        result.append({
            "id": row["id"],
            "nome": row["nome"],
...
def get_produto_por_id(id):
    db = get_db()
    cursor = db.cursor()
    # SMELL: SQL Injection - concatenação de string direto na query
...
# SMELL: Código completamente duplicado para usuarios (copy-paste dos produtos)
def get_todos_usuarios():
    ...
    for row in rows:
        result.append({
            ...
            "nome": row["nome"],
            "email": row["email"],
            "senha": row["senha"],  # SMELL: Expondo senha na resposta!
            "tipo": row["tipo"],
            "criado_em": row["criado_em"]
        })
    return result
...
def get_usuario_por_id(id):
    ...
    if row:
        return {
            ...
            "nome": row["nome"],
            "email": row["email"],
            "senha": row["senha"],  # SMELL: Expondo senha
            "tipo": row["tipo"],
            "criado_em": row["criado_em"]
        }
    return None
...
def criar_usuario(nome, email, senha, tipo="cliente"):
    db = get_db()
    cursor = db.cursor()
    # SMELL: Sem verificar email duplicado, sem hash de senha
    cursor.execute(
...
# SMELL: Lógica de negócio complexa no model
def criar_pedido(usuario_id, itens):
    db = get_db()
    cursor = db.cursor()

    total = 0
    # SMELL: N+1 query problem - busca cada produto individualmente
    for item in itens:
    ...
    # SMELL: Sem transaction - se falhar no meio, dados ficam inconsistentes
    cursor.execute(
    ...
    for item in itens:
        ...
        produto = cursor.fetchone()  # SMELL: Buscando o mesmo produto DE NOVO
        cursor.execute(
...
def get_todos_pedidos():
    ...
    result = []
    for row in rows:
        # SMELL: Copy-paste da função acima
        pedido = {
            "id": row["id"],
            "usuario_id": row["usuario_id"],
            "status": row["status"],
...
# SMELL: "Relatório" com lógica de negócio pesada no model
def relatorio_vendas():
    ...
    # SMELL: Cálculo de desconto com magic numbers
    desconto = 0
    ...
    # SMELL: Formatação de resposta misturada com lógica
    return {
...
def atualizar_status_pedido(pedido_id, novo_status):
    db = get_db()
    cursor = db.cursor()
    # SMELL: Sem validação de transição de status
    # SMELL: Permite qualquer string como status
    cursor.execute(
...
# SMELL: Busca com lógica duplicada e ineficiente
def buscar_produtos(termo, categoria=None, preco_min=None, preco_max=None):
    db = get_db()
    cursor = db.cursor()

    # SMELL: Construção dinâmica de SQL sem parameterização
    ...
    result = []
    for row in rows:
        # SMELL: Mesma conversão copiada pela 4ª vez
        result.append({
            "id": row["id"],
            "nome": row["nome"],
            "descricao": row["descricao"],
