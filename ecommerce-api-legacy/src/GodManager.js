...
class GodManager {
    constructor() {
        // Acoplamento extremo e banco em memória
    ...
    setupRoutes(app) {
        // [ERRO] God Method + Callback Hell (Pyramid of Doom) + Variáveis péssimas
        app.post('/api/checkout', (req, res) => {
            let u = req.body.usr; // Nome
            let e = req.body.eml; // Email
            let p = req.body.pwd; // Senha
            let cid = req.body.c_id; // Curso
            let cc = req.body.card; // Cartão fake

            if (!u || !e || !cid || !cc) return res.status(400).send("Bad Request");

            // Nível 1: Busca o curso
                ...
                this.db.get("SELECT id FROM users WHERE email = ?", [e], (err, user) => {
                    ...
                    let processPaymentAndEnroll = (userId) => {
                        ...
                        this.db.run("INSERT INTO enrollments (user_id, course_id) VALUES (?, ?)", [userId, cid], function(err) {
                            ...
                            // Nível 6: Salva Pagamento
                            this.db.run("INSERT INTO payments (enrollment_id, amount, status) VALUES (?, ?, ?)", [enrId, course.price, status], function(err) {
                                if (err) return res.status(500).send("Erro Pagamento");

                                // Nível 7: Audit log
                                this.db.run("INSERT INTO audit_logs (action, created_at) VALUES (?, datetime('now'))", [`Checkout curso ${cid} por ${userId}`], (err) => {
                    ...
                    if (!user) {
                        // Nível 3: Cria usuário se não existir
        ...
        // [ERRO CRÍTICO] O Pesadelo do N+1 e N*M Queries
        app.get('/api/admin/financial-report', (req, res) => {
            let report = [];

            // Query 1: Pega todos os cursos
            this.db.all("SELECT * FROM courses", [], (err, courses) => {
                ...
                courses.forEach(c => {
                    ...
                    // Query 2 (Dentro do loop): Pega matrículas do curso
                    this.db.all("SELECT * FROM enrollments WHERE course_id = ?", [c.id], (err, enrollments) => {
                        ...
                        enrollments.forEach(enr => {
                            // Query 3 (Dentro do loop do loop): Pega usuário
                            this.db.get("SELECT name, email FROM users WHERE id = ?", [enr.user_id], (err, user) => {
                                
                                // Query 4 (Dentro do loop do loop): Pega pagamento
        ...
        // [ERRO] Deleção sem integridade referencial ou transação
        app.delete('/api/users/:id', (req, res) => {
            let id = req.params.id;
            this.db.run("DELETE FROM users WHERE id = ?", [id], (err) => {
                // Deixa matrículas órfãs de propósito
