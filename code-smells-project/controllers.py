...
# SMELL: Controllers fazendo validação, lógica de negócio, formatação, tratamento de erro - tudo junto
# SMELL: Sem autenticação/autorização em nenhum endpoint
# SMELL: Sem padronização de respostas de erro
# SMELL: Print ao invés de logging
...
def listar_produtos():
    try:
        ...
        print("Listando " + str(len(produtos)) + " produtos")  # SMELL: print como log
        return jsonify({"dados": produtos, "sucesso": True}), 200
    except Exception as e:
        print("ERRO: " + str(e))
        return jsonify({"erro": str(e)}), 500  # SMELL: Expondo erro interno
...
def criar_produto():
    try:
        ...
        # SMELL: Validação manual, verbosa e incompleta
        if not dados:
        ...
        # SMELL: Validação de negócio no controller
        if preco < 0:
        ...
        # SMELL: Lógica de categorias válidas hardcoded no controller
...
def atualizar_produto(id):
    try:
        ...
        # SMELL: Busca o produto mas não usa o resultado para nada útil
        produto_existente = models.get_produto_por_id(id)
        if not produto_existente:
            return jsonify({"erro": "Produto não encontrado"}), 404

        # SMELL: Validação copy-paste do criar_produto
        if not dados:
            return jsonify({"erro": "Dados inválidos"}), 400
        if "nome" not in dados:
            return jsonify({"erro": "Nome é obrigatório"}), 400
...
def deletar_produto(id):
    try:
        # SMELL: Deleta sem verificar se tem pedidos associados
        produto = models.get_produto_por_id(id)
        if not produto:
            return jsonify({"erro": "Produto não encontrado"}), 404
...
def buscar_produtos():
    try:
        ...
        # SMELL: Conversão sem tratamento
        if preco_min:
...
def listar_usuarios():
    try:
        ...
        # SMELL: Retorna TODOS os dados incluindo senhas!
...
def criar_usuario():
    try:
        ...
        # SMELL: Validação mínima e diferente das outras entidades
        nome = dados.get("nome", "")
        ...
        # SMELL: Senha armazenada em texto puro
...
def login():
    try:
        ...
        if usuario:
            # SMELL: Sem JWT, sem session, sem nada - retorna dados e pronto
...
def criar_pedido():
    try:
        ...
        # SMELL: Não verifica se o usuário existe antes de criar o pedido
        resultado = models.criar_pedido(usuario_id, itens)

        if "erro" in resultado:
            return jsonify({"erro": resultado["erro"], "sucesso": False}), 400

        # SMELL: Lógica de notificação/email misturada no controller
...
def listar_todos_pedidos():
    try:
        # SMELL: Qualquer um pode ver todos os pedidos - sem controle de acesso
...
def atualizar_status_pedido(pedido_id):
    try:
        ...
        # SMELL: Validação de status hardcoded e diferente do que deveria ser uma máquina de estados
        ...
        # SMELL: Side effects espalhados pelo controller
        ...
        if novo_status == "cancelado":
            ...
            # SMELL: Deveria devolver o estoque mas não devolve!
...
# SMELL: Endpoint "utilitário" que acessa o banco direto, bypassando o model
def health_check():
    try:
        db = get_db()
        cursor = db.cursor()
        ...
        return jsonify({
            ...
            # SMELL: Expondo versão e informações internas
            "versao": "1.0.0",
            "ambiente": "producao",
            "db_path": "loja.db",
            "debug": True,
            "secret_key": "minha-chave-super-secreta-123"  # SMELL: Expondo secret key!
        }), 200
    except Exception as e:
