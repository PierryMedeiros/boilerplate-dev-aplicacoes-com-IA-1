...
def create_user():
    ...
    # validar senha (mínimo 4 caracteres - muito fraco!)
    if len(password) < 4:
...
def delete_user(user_id):
    ...
    # deletar tasks do usuario antes
    tasks = Task.query.filter_by(user_id=user_id).all()
    for t in tasks:
        db.session.delete(t)
...
def login():
    ...
    # retornar dados do usuario sem token real
    return jsonify({
